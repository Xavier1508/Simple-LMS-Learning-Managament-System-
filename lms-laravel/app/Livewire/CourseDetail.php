...
class CourseDetail extends Component
{
    ...
    public function mount($id)
    {
        ...
        // Otomatis buka sesi yang paling relevan (sedang berlangsung atau yang akan datang terdekat)
        $now = Carbon::now();
    ...
    // --- FITUR ATTENDANCE (LOGIKA DIPERBAIKI) ---
    public function attend($sessionId)
    {
        ...
        // ATURAN WAKTU:
        // Buka: 30 menit SEBELUM start_time
        // Tutup: 15 menit SETELAH start_time
        $openTime = $session->start_time->copy()->subMinutes(30);
        $closeTime = $session->start_time->copy()->addMinutes(15);

        // Cek apakah dalam rentang waktu
        if ($now->lt($openTime)) {
            session()->flash('error', 'Attendance not yet open. You can check in 30 minutes before class starts.');
            return;
        }

        if ($now->gt($closeTime)) {
            session()->flash('error', 'Attendance is closed. The check-in period has ended.');
            return;
        }

        if (!$session->isAttendedBy($user->id)) {
            Attendance::create([
            ...
            session()->flash('message', 'Success! Your attendance has been recorded.');
        } else {
            session()->flash('info', 'You have already checked in for this session.');
        }
    }

    // Helper untuk cek status waktu absen di View
    public function getAttendanceStatus($sessionId)
    {
        $session = CourseSession::find($sessionId);
        ...
        if ($now->lt($openTime)) return 'upcoming'; // Belum mulai
        if ($now->gt($closeTime)) return 'closed';  // Sudah lewat
        return 'open'; // Sedang berlangsung
    }

    // --- FITUR UPLOAD MATERIAL (DIPERBAIKI KEAMANAN) ---
    ...
    {
        // Validasi File yang lebih ketat & aman
        // Support format: dokumen kantor, PDF, dan gambar
        $this->validate([
            'newFile' => [
                'required',
                'file',
                ...
                function ($attribute, $value, $fail) {
                    // Cek MIME type yang sebenarnya (bukan hanya ekstensi)
                    $allowedMimes = [
                        'application/pdf',
                        'application/msword',
            ...
            'newFile.mimes' => 'Only PDF, Office documents (Word, PowerPoint, Excel), text files, and images are allowed.',
            'newFile.max' => 'File size must not exceed 10MB.'
        ]);

        // Sanitize filename untuk menghindari path traversal
    ...
    public function downloadMaterial($materialId)
    {
        ...
        if ($material && Storage::disk('public')->exists($material->file_path)) {
            // Menggunakan response download dari Storage facade
    ...
    {
        $material = CourseMaterial::find($materialId);
        // Pastikan hanya dosen kelas ini yang bisa hapus
    ...
    public function previewMaterial($materialId)
    {
        $material = CourseMaterial::find($materialId);
        if ($material) {
            $this->previewFileUrl = Storage::url($material->file_path);
            $this->previewFileType = strtolower($material->file_type);
            $this->previewFileName = $material->file_name;
    ...
    {
        $courseClass = CourseClass::with(['course', 'lecturer', 'sessions.materials', 'sessions.attendances'])
